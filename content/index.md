...
Some commands return data, some simply perform an action.

### Action Commands

Action commands return any output that is needed, even if it is just "OK". They also return an appropriate exit status.

All non-data-returning commands support a `--quiet` switch to suppress output.
...
For commands that return multiple values, adding a `--format-json` switch will return a pretty-printed JSON response instead of the standard data output.
...
The tool must autocomplete, including offering suggestions for partial inputs

 * topics, subtopics and commands
 * argument flags
 * known values such as application IDs
